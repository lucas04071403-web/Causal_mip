    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                ...
                # 图片特征, Image_encoder
                image_embeds, _, _ = self.visual(pixel_values, grid_thw=image_grid_thw)
...
class PatchMerger(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        resized_x = self.ln_q(x).view(-1, self.hidden_size)
        output = self.mlp(resized_x)
        return output, resized_x, x
...
class Qwen2VisionTransformerPretrainedModel(Qwen2VLPreTrainedModel):
    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        output, resized_x, x = self.merger(hidden_states)
