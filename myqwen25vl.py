    ...
    def forward(self, hidden_states: torch.Tensor, grid_thw: torch.Tensor) -> torch.Tensor:
        ...
        for layer_num, blk in enumerate(self.blocks):
            ...
            if self.gradient_checkpointing and self.training:
                block_hidden_states = self._gradient_checkpointing_func(
                    blk.__call__, hidden_states, cu_seqlens_now, None, position_embeddings
                )
            else:
                block_hidden_states = blk(hidden_states, cu_seqlens=cu_seqlens_now, position_embeddings=position_embeddings)

        hidden_states = self.merger(block_hidden_states)
        reverse_indices = torch.argsort(window_index)
        hidden_states = hidden_states[reverse_indices, :]

        return hidden_states, block_hidden_states
...
class Qwen2_5_VLModel(Qwen2_5_VLPreTrainedModel):
    ...
    def get_video_features(
        ...
        video_embeds, block_hidden_states = self.visual(pixel_values_videos, grid_thw=video_grid_thw)
        return video_embeds, block_hidden_states
    ...
    def get_image_features(self, pixel_values: torch.FloatTensor, image_grid_thw: Optional[torch.LongTensor] = None):
        ...
        image_embeds, block_hidden_states = self.visual(pixel_values, grid_thw=image_grid_thw)
        return image_embeds, block_hidden_states
    ...
    def forward(
        ...
        if inputs_embeds is None:
            ...
            if pixel_values is not None:
                image_embeds, block_hidden_states = self.get_image_features(pixel_values, image_grid_thw)
            ...
            if pixel_values_videos is not None:
                video_embeds, block_hidden_states = self.get_video_features(pixel_values_videos, video_grid_thw)
        ...
        output = Qwen2_5_VLModelOutputWithPast(
            ...
            visual_hidden_states=image_embeds,
        )
